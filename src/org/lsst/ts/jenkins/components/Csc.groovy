...
def build_idl_conda(label) {
    ...
    timeout(5) {
        waitUntil(initialRecurrencePeriod: 15000, quiet: true) {
            ...
            def r = sh (
                script: "yum clean all ; yum list -y --enablerepo=${rpm_repo} ts_sal_runtime-${params.XML_Version}-${params.SAL_Version}.el9.x86_64 ",
                returnStatus: true
            )
            return r == 0
        }
    ...
    sh """
        ...
        yum install -y --enablerepo=${rpm_repo} ts_sal_runtime-${params.XML_Version}-${params.SAL_Version}.el9.x86_64
        cd ${WHOME}/conda
        source /home/saluser/.setup.sh
        conda config --set solver libmamba
        conda config --add channels conda-forge
